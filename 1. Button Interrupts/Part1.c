...
int risingEdge, fallingEdge;                // Changing conditions
void gpioInit();
...
__interrupt void Port_2(void)
{
    ...
    if(P2IES & BIT3)        //There is a rising edge
    {
        LED_Color = 0;
        P1OUT &= ~BIT0; //Red led off
        P2IES &= ~BIT3; //Making this a falling edge
    }

    else if(P2IES != BIT3) //There isn't a falling edge
    {
        LED_Color = 1;
        P6OUT &= ~BIT6;     //Green led off
        P2IES |= BIT3;      //Now we check for a rising edge instead
    }
}
